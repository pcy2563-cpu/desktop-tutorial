...
export default function FAQSection() {
  ...
  return (
    <section className="qx-section-surface py-24 md:py-40 relative overflow-hidden">
            ...
            {faqItems.map(
              (item: { question: string; answer: string }, index: number) => (
                <AccordionItem 
                  key={index} 
                  value={`item-${index}`}
                  className="qx-faq-item border border-border/50 rounded-2xl px-6 bg-secondary/20 hover:bg-secondary/40 transition-colors overflow-hidden"
                >
